...
| **Quarantine** — move every non-matching-extension file to `_quarantine/excluded/` before scanning, and every file that fails to parse to `_quarantine/bad_files/` after | `--quarantine` | [Quarantine](#quarantine) |
...
1. **Run it.** A first run over a fresh root produces `strings.txt` (every
   ...
   `bad_files.tsv` recording the reason for any file that fails to parse.
   Pass `--quarantine` to also physically move any non-matching-extension
   file to `_quarantine/excluded/` and any file that fails to parse to
   `_quarantine/bad_files/` — see [Quarantine](#quarantine); without it,
   both kinds of file are simply left where they are.
...
3. **Triage `bad_files.tsv`.** For each file listed, open it at its
   `quarantined_file` path if `--quarantine` is in use (it's no longer at
   its original location — see [Quarantine](#quarantine)), or at its
   original path otherwise; fix whatever made it unparseable (or decide
   it's fine to leave out), then make sure it's sitting somewhere under
...
4. **Re-run.** Names now in `known_names.txt`/`stopwords.txt` change
   ...
   again and either scans clean or lands back in the list (and, with
   `--quarantine`, back in `_quarantine/bad_files/`) with a reason.
...
| `--quarantine` | off | Physically move non-matching-extension files to `_quarantine/excluded/` before scanning, and files that fail to parse to `_quarantine/bad_files/` after — see [Quarantine](#quarantine). Off by default: moving files out of the tree is destructive and hard to reverse, so it's opt-in rather than automatic. With it off, neither move changes what gets scanned or what's recorded in `bad_files.tsv`/`quarantine_manifest.tsv` — it only changes whether the file physically leaves `root` |
...
| `bad_files.tsv` | always, merged with past runs | Persistent record of files that matched `--extensions` and were actually attempted, but too malformed to parse — one row per file (original path, category, item, reason, quarantined-to path), tab-separated. With `--quarantine` on, each run's newly-flagged files are also physically moved to `_quarantine/bad_files/` (see [Quarantine](#quarantine)) and the row's quarantined-to path recorded; without it, the row is still added but that column stays empty and the file is left at its original path. Either way entries survive until you delete their row — see [Bad files](#bad-files). Not to be confused with `quarantine_manifest.tsv` below, whose files were never attempted at all |
...
`--quarantine` (off by default) physically moves two kinds of file out of
the scanned tree and into `_quarantine/`, so that after a run, everything
left under `root` is exactly what was actually scanned, and everything
that wasn't (or that was but couldn't be made sense of) is sitting
somewhere else, still on disk, never deleted. It's opt-in rather than
automatic because moving files off disk out from under `root` is a
destructive, hard-to-reverse action a scan shouldn't take without being
asked — with the flag off, both kinds of file are simply left at their
original path (a bad file's `bad_files.tsv` row is still written, just
with an empty `quarantined_file` column):
...
whenever `--quarantine` is on. `--verbose` adds a second, indented line
under each: the extension that excluded it, or the bad-file
category/reason.
...
offending value/snippet, and a full reason in `bad_files.tsv`. With
`--quarantine` on, the file itself is also physically moved to
`_quarantine/bad_files/` (see [Quarantine](#quarantine) above) rather
than just recorded and left in place. Either way, on every later run it's
skipped entirely — not even attempted — purely because its path is
already listed in `bad_files.tsv`, whether or not it was actually moved;
putting it back up for another attempt means fixing it (at its
`quarantined_file` path if it was moved, its original path otherwise),
making sure it's sitting somewhere under `root` matching `--extensions`,
and deleting its data row (keeping the header row) so the next scan picks
it up fresh. See [Workflow](#workflow) above for how this fits into the
full triage loop.
