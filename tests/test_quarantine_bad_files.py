...
wiring -- this runs only when --quarantine is passed, and bad_files.tsv
keeps the row (original path + reason) with a quarantined_file column
...
class TestQuarantineBadFilesCLI(unittest.TestCase):
    ...
    def test_bad_file_is_quarantined_when_flag_passed(self):
    ...
    def test_without_flag_bad_file_is_recorded_but_left_in_place(self):
        self._run([self.sql_root, "--extensions", "sql"])

        self.assertTrue(os.path.isfile(os.path.join(self.sql_root, "bad.sql")))
        self.assertFalse(os.path.exists(os.path.join("_quarantine", "bad_files")))

        entries = load_bad_files("bad_files.tsv")
        original_abspath = os.path.abspath(os.path.join(self.sql_root, "bad.sql"))
        self.assertIn(original_abspath, entries)
        self.assertEqual(entries[original_abspath].quarantined_file, "")
    ...
    def test_bad_file_not_rescanned_from_quarantine_on_next_run(self):
        ...
        code = self._run([self.sql_root, "--extensions", "sql", "--quarantine"])
    ...
    def test_bad_file_not_rescanned_on_next_run_even_without_flag(self):
        # Without --quarantine, bad.sql is still sitting at its original
        # path (never moved) -- exclude_paths must still skip it on the
        # next run purely from bad_files.tsv, not from it being physically
        # missing.
        self._run([self.sql_root, "--extensions", "sql"])
        code = self._run([self.sql_root, "--extensions", "sql"])
        self.assertEqual(code, 0)  # only good.sql re-scanned, no findings
