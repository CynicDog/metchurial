# -*- coding: utf-8 -*-
"""Tests for quarantine.extensions.quarantine_non_matching's own
file-moving logic, plus the CLI wiring: it runs only when --quarantine is
passed, moving every non-matching-extension file to _quarantine/excluded/
and writing quarantine_manifest.tsv.
...
class TestQuarantineNonMatching(unittest.TestCase):
    ...
    def test_moves_non_matching_recursively_preserving_relative_path(self):
        ...
        excluded_dir = os.path.join(self.root, "_quarantine", "excluded")

        rows = quarantine.quarantine_non_matching(self.root, ("sql",), excluded_dir)

        moved = {os.path.relpath(r.quarantined_file, excluded_dir) for r in rows}
    ...
    def test_does_not_descend_into_quarantine_root_itself(self):
        self._write("notes.docx")
        excluded_dir = os.path.join(self.root, "_quarantine", "excluded")

        first = quarantine.quarantine_non_matching(self.root, ("sql",), excluded_dir)
        self.assertEqual(len(first), 1)
        # Re-running must not re-discover the file it just moved into
        # _quarantine/excluded (which sits inside root) and move it again
        # -- nor treat _quarantine/bad_files (a sibling under the same
        # _quarantine root) as fair game either.
        second = quarantine.quarantine_non_matching(self.root, ("sql",), excluded_dir)
        self.assertEqual(second, [])

    def test_repeated_move_to_same_relative_path_does_not_clobber(self):
        excluded_dir = os.path.join(self.root, "_quarantine", "excluded")
        self._write("notes.docx", content="first")
        quarantine.quarantine_non_matching(self.root, ("sql",), excluded_dir)
        ...
        original_dest = os.path.join(excluded_dir, "notes.docx")
    ...
    def test_exclude_paths_are_never_moved(self):
        ...
        rows = quarantine.quarantine_non_matching(
            self.root, ("sql",), excluded_dir, exclude_paths={os.path.abspath(excluded)})
...
class TestQuarantineCLI(unittest.TestCase):
    ...
    def test_quarantine_flag_moves_non_matching_and_writes_manifest(self):
        ...
        self.assertTrue(os.path.isfile(
            os.path.join("_quarantine", "excluded", "notes.docx")))
        self.assertTrue(os.path.isfile(
            os.path.join("_quarantine", "excluded", "sub", "readme.md")))
    ...
    def test_without_flag_non_matching_files_are_left_in_place(self):
        self._run([self.sql_root, "--extensions", "sql"])

        self.assertTrue(os.path.isfile(os.path.join(self.sql_root, "notes.docx")))
        self.assertTrue(os.path.isfile(os.path.join(self.sql_root, "sub", "readme.md")))
        self.assertTrue(os.path.isfile(os.path.join(self.sql_root, "a.sql")))
        self.assertFalse(os.path.exists(os.path.join("_quarantine", "excluded")))
    ...
    def test_scan_root_as_cwd_does_not_quarantine_own_output_artifacts(self):
        ...
        self.assertTrue(os.path.isfile(os.path.join("_quarantine", "excluded", "notes.docx")))
    ...
    def test_does_not_quarantine_its_own_running_script(self):
        ...
        self.assertFalse(os.path.exists(
            os.path.join("_quarantine", "excluded", "metchurial.py")))
