...
class TestKnownGrammarGap(unittest.TestCase):
    """COUNT/MAX/LOWER are reserved keywords in the vendored grammar, not
    plain identifiers -- function_name can never match them. See
    function_visitor.py's module docstring."""

    def test_count_is_not_captured(self):
        fc = function_calls_for("SELECT COUNT(col1) FROM t1;")
        self.assertNotIn("COUNT", [f["function"] for f in fc])

    def test_max_is_not_captured(self):
        fc = function_calls_for("SELECT MAX(col1) FROM t1;")
        self.assertNotIn("MAX", [f["function"] for f in fc])

    def test_sum_and_avg_are_captured(self):
        # SUM/AVG/MIN are *not* reserved keywords in this grammar (unlike
        # COUNT/MAX/LOWER) -- they lex as plain ID and parse normally.
