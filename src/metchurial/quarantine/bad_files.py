# -*- coding: utf-8 -*-
"""When --quarantine is passed, after the scan itself runs, physically
moves every file it flagged bad this run (models/bad_file.py's
BadFileReason, one per tree.bad_files entry -- see engine.py) out of the
scanned tree and into _quarantine/bad_files/, mirroring its path relative
to root the same way quarantine.extensions does for _quarantine/excluded/.
Only runs when --quarantine is passed (see cli.py): a bad file already
didn't get scanned for sensitive content, so with the flag on, physically
moving it out of a tree that's about to be compressed and shipped outside
the company is the point. With the flag off, it's still recorded in
bad_files.tsv (with an empty quarantined_file) but left at its original
path -- moving files out of the tree unprompted is a destructive action
this project no longer takes without the caller asking for it.
