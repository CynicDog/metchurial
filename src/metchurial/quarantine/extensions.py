# -*- coding: utf-8 -*-
"""When --quarantine is passed, before the scan itself runs, recursively
moves every file under the scan root whose extension isn't in
--extensions out into _quarantine/excluded/ -- so after a run, everything
left under root matches one of --extensions, and every file that didn't
is sitting in excluded_dir instead, still grouped by the folder it came
from. Only runs when --quarantine is passed (see cli.py) -- with the flag
off, non-matching files are simply left where they are, since the scan
itself already only looks at --extensions files regardless.
