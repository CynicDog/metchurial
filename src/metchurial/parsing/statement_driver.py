...
from metchurial.parsing.statement_starts import subdivide
...
def chunk_ranges(all_tokens: list[Token],
                 guard_hits: dict[str, int] | None = None) -> list[tuple[int, int]]:
    """[start, end) index ranges into all_tokens, one per statement.

    Two passes. First split at every top-level ';' (_semi_chunk_ranges),
    then subdivide each of those chunks at inferred statement starts
    (statement_starts.subdivide), because a ';' is DB2 syntax for ending
    a statement but is not reliably present: plenty of real .sql files
    keep several standalone queries side by side, never terminated,
    because the author highlights the one they want and runs the
    selection. Without the second pass such a file arrives here as one
    chunk and every consumer -- the tiered parse below, table/JOIN
    discovery, query identity, --split-selects -- treats N unrelated
    queries as a single statement.

    Subdivision only ever runs *within* an already-';'-delimited chunk,
    so a correctly punctuated file is unaffected and a half-punctuated
    one works either way. See parsing/statement_starts.py for the
    boundary rule and its guards. `guard_hits`, if given, accumulates how
    many times each guard (models/statement_boundary.py) stopped a
    candidate keyword from becoming a boundary."""
    out: list[tuple[int, int]] = []
    for start, end in _semi_chunk_ranges(all_tokens):
        out.extend(subdivide(all_tokens, start, end, guard_hits=guard_hits))
    return [r for r in out if r[1] > r[0] and not _is_eof_only(all_tokens, r)]


def semicolon_boundary_count(all_tokens: list[Token]) -> int:
    """How many of chunk_ranges()'s boundaries came from a real ';'.
    engine.py subtracts this from the chunk count to report how many were
    inferred instead -- the one number that makes the inference above
    auditable at a glance."""
    return len(_semi_chunk_ranges(all_tokens))


def _semi_chunk_ranges(all_tokens: list[Token]) -> list[tuple[int, int]]:
...
def parse_file(text: str, visitor: Any,
    ...
    ranges = chunk_ranges(all_tokens,
                          guard_hits=stats.guard_hits if stats is not None else None)
    if stats is not None:
        # Every chunk beyond what ';' alone would have produced came from
        # statement_starts' inference -- the one number that makes it
        # auditable. Counted here rather than in parse_chunk because it's
        # a property of the file's split, not of any one chunk.
        stats.inferred_boundaries += max(
            0, len(ranges) - semicolon_boundary_count(all_tokens))
