...
--quarantine (off by default) physically moves every file under root
whose extension isn't in --extensions into _quarantine/excluded/ before
the scan starts, and moves every file the scan itself flags bad into
_quarantine/bad_files/ once it's done -- see metchurial.quarantine's own
module docstring. Off by default because moving files out of the tree
is a destructive, hard-to-reverse action a scan shouldn't take without
the caller explicitly asking for it; with the flag off, non-matching and
bad files are simply left in place (bad ones still recorded in
bad_files.tsv, just with an empty quarantined_file).
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Parse DB2 SQL into structured metadata (table/column/function/"
                    ...
                    "(and, by default, .txt) files. Pass --quarantine to also "
                    "physically move non-matching-extension files into "
    ...
    ap.add_argument("--quarantine", action="store_true",
                    help="Physically move files out of root instead of leaving them "
                        ...
                        "in --extensions moves into _quarantine/excluded/; after the "
                        "scan, every file it flags bad moves into "
                        "_quarantine/bad_files/. Off by default -- neither move changes "
                        "what gets scanned (the scan already only looks at "
                        "--extensions files, and a bad file's bad_files.tsv row is "
                        "recorded either way), it only changes whether the file "
                        "physically leaves root. See README's Quarantine section "
                        "(default: off)")
    ...
    # Runs before the scan itself, gated behind --quarantine: once this
    # returns, everything left under args.root matches --extensions, so
    # scan_tree sees exactly the same tree it would have anyway -- this
    # changes what's left lying around afterward, not what gets scanned
    # (the scan already only looks at --extensions files regardless of
    # whether this ran). With --quarantine off, non-matching files are
        ...
        if result.bad_reason is not None:
            ...
            print("        antlr: {} chunk(s){}, {} iteration(s) "
                 "(struct={} scan={} resync={} skip={}), {:.3f}s".format(
                     s.chunks,
                     " ({} inferred)".format(s.inferred_boundaries)
                     if s.inferred_boundaries else "",
                     s.iterations, s.tier1_structural, s.tier1_token_scan,
                     s.tier2_resync, s.tier3_skip, s.elapsed_seconds), file=sys.stderr)
            if s.guard_hits:
                # Which boundary guards actually fired on this file (see
                # models/statement_boundary.py) -- only printed when at
                # least one did, so a conventionally punctuated file
                # stays quiet.
                print("        guards: {}".format(" ".join(
                    "{}={}".format(k, s.guard_hits[k]) for k in sorted(s.guard_hits))),
                    file=sys.stderr)
    ...
    # Gated behind --quarantine: a file that just failed to parse was never
    # checked for sensitive content either, so with the flag on, physically
    # moving it out of a tree about to be compressed and shipped outside
    # the company is the point. With the flag off, tree.bad_files is used
    # as-is (each BadFileReason's quarantined_file already defaults to "")
    # -- the file stays at its original path, still recorded in
    # bad_files.tsv. Only tree.bad_files (flagged *this* run) is ever
    # moved -- a previously_bad entry was never re-scanned, so it isn't
    # still sitting at its original path to move in the first place (see
    # quarantine.bad_files's own docstring).
    if args.quarantine:
        ...
        newly_quarantined_bad = tree.bad_files
    ...
    run_info = {
        ...
        "inferred_boundaries": tree.inferred_boundaries,
        ...
        "mask_literals": args.mask_literals, "quarantine": args.quarantine,
    ...
    if args.quarantine:
        ...
        print("Excluded       : --quarantine not passed -- non-matching files left in place")
    ...
    if previously_bad or tree.bad_files:
        if args.quarantine:
            ...
            print("Bad files      : {} skipped (already in bad_files.tsv), {} newly flagged "
                 "this run (left in place -- pass --quarantine to move them) -- see {}".format(
                     len(previously_bad), len(tree.bad_files),
                     os.path.abspath(BAD_FILES_PATH)))
