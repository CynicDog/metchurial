...
Contrast with quarantine.extensions.quarantine_non_matching (runs only
when --quarantine is passed, moving files into _quarantine/excluded/): a
...
class BadFileReason:
    ...
    quarantine_bad_files actually moves it, which only happens when
    --quarantine is passed (never re-populated for an entry carried over
    unchanged from a previous run's bad_files.tsv)."""
