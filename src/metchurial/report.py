...
def _write_run_info(out: TextIO, run_info: dict[str, Any]) -> None:
    ...
    # Only worth a row when it's non-zero: a conventionally ';'-terminated
    # tree infers nothing, and a row of "0" would just be noise. When it is
    # non-zero it's the number to check first if a statement looks
    # mis-split -- see parsing/statement_starts.py.
    if run_info.get("inferred_boundaries"):
        out.write("| Statement boundaries inferred (no `;`) | {} |\n".format(
            run_info["inferred_boundaries"]))
...
def _write_bad_files(out: TextIO, previously_bad: dict[str, BadFileReason],
                     new_bad: dict[str, BadFileReason], quarantine: bool) -> None:
    out.write("## Bad Files\n\n")
    ...
    if quarantine:
        ...
        out.write("{} skipped (already in bad_files.tsv), {} newly flagged this run "
                  "(left in place -- pass --quarantine to move them). Full list in "
...
def write_markdown_report(path: str, run_info: dict[str, Any], tree: TreeScanResult, *,
    ...
    `run_info`: {invocation, root, file_count, inferred_boundaries,
    sensitive_columns,
    ...
    mask_literals, quarantine, verbose}."""
